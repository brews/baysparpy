...
def predict_tex_analog(seatemp, lat, lon, temptype, stol, nens=5000): 
    ...
    lat : float Site latitude from -90 to 90.
    lon : float Site longitude from -180 to 180.
    temptype : str Type of sea temperature used. Either 'sst' for sea-surface or 'subt'.
    stol: float search tolerance in seatemp units (required for analog mode)
    nens : int Size of MCMC ensemble draws to use for calculation.
    Returns
    -------
    output : Prediction
    Raises
    ...
    EnsembleSizeError
    """
    draws_analog = get_draws_analog(temptype)
    # number of big grids:
    N_bg = draws_analog.data_input['Locs'][0,0].shape[0]
    
    nd = len(seatemp)
    t = seatemp
    
    inder_g = np.empty((0,1), int)
    ...
    for kk in range(0,N_bg):
        ...
        if np.mean(vals) >= (np.mean(t)-stol) && np.mean(vals)<=(np.mean(t)+stol) :
            #inder_g = [inder_g; kk]
            inder_g = np.append(inder_g, np.array([[kk]]), axis=0)
    if inder_g.size == 0:
        print('No analogs were found. Make your search tolerance wider.')
        sys.exit('No analogs were found. Make your search tolerance wider.')
    
    
    alpha_samples = draws_analog.alpha_samples[inder_g]
    beta_samples = draws_analog.beta_samples[inder_g]
    tau2_samples = 
    # reshape
    alpha_samples =
    beta_samples = 
    # downsample to 5000
    iters=len(alpha_samples)
    ds=round(iters/5000)
    alpha_samples = 
    beta_samples = 
    tau2_samples = 
